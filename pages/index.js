        ...
        for (let i = 1; i <= allSections.length - 1; i++) {
          window.addEventListener("scroll", (e) => {
            if (window.innerHeight * i - window.pageYOffset >= 0) {
              allSections[i].style.top = window.innerHeight * i - window.pageYOffset + "px";
            }
            if (
              ...
              var temp = setInterval(() => {
                if (
                  ...
                  allSections[i].style.top = parseInt(allSections[i].style.top.replace("px", "")) - 1 + "px";
            ...
            if (
              parseInt(allSections[i].style.top.replace("px", "")) < window.innerHeight &&
              parseInt(allSections[i].style.top.replace("px", "")) > window.innerHeight - 50
            ) {
              var temp = setInterval(() => {
                if (
                  parseInt(allSections[i].style.top.replace("px", "")) < window.innerHeight &&
  ...
  return (
    <div id="main">
        ...
        <>
          <div id="popup">
            <div>
              ...
              />
              <span>View this website on a Deskop or a Laptop for a better viewing experience.</span>
            </div>
          </div>
        </>
      )}
      ...
      <>
        ...
        <Experience />
        <Work />
        {extra_heading !== null && <Extra res={extra_heading.length != 0 ? photographyData : []} />}
...
export async function getServerSideProps(context) {
  var res = await axios.get("https://api.github.com/repos/AishwaryaT19/My-Portfolio");
  cloudinary.config({
